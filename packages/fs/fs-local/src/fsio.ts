...
function isHardLinkUnsupported(error: unknown): boolean {
  return error instanceof Error && 'code' in error
    && (error.code === 'EACCES' || error.code === 'EPERM' || error.code === 'ENOTSUP'
        || error.code === 'EOPNOTSUPP' || error.code === 'ENOSYS' || error.code === 'EXDEV')
}
...
export interface FsIoInternals {
  ...
  /** Override the no-replace copy fallback used when hard links are unavailable. */
  copyFileExcl?: (source: string, destination: string) => Promise<void>
...
export async function writeFileAtomic(
  ...
  const copyFileExcl = internals.copyFileExcl
  ...
  try {
    ...
    if (createIfAbsent !== undefined) {
      try {
        ...
        // Some filesystems (notably Android's) deny hard links with EACCES/EPERM even
        // though the same directory is writable. Fall back to a no-replace copy, which
        // preserves create-if-absent semantics without the hard-link primitive.
        if (isHardLinkUnsupported(error)) {
          try {
            await copyFileExcl(tempPath, absolutePath)
